    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-center">
      {services.map((service, index) => (
        <div key={index} className="bg-white shadow-lg p-6 rounded-lg">
          <button
            className="text-6xl mb-4 text-center hover:scale-110 transition-transform duration-200"
            aria-label={service.title}
            onClick={() => alert(`${service.title} Clicked!`)} // Example action, replace with your desired functionality
          >
            {service.icon}
          </button>
